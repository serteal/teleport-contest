...
function writeCandidate(base, movesLength, outDir) {
  ...
  const input = join(
    outDir,
    `candidate-${String(movesLength).padStart(5, "0")}.input.session.json`,
  );
  const output = join(
    outDir,
    `candidate-${String(movesLength).padStart(5, "0")}.session.json`,
  );
...
function record(input, output) {
  ...
  if (child.status !== 0) {
    throw new Error(
      `record failed for ${basename(input)}\n${child.stderr || child.stdout}`,
    );
  }
}
...
function check(output, mode) {
  const child = spawnSync(
    process.execPath,
    [checkScript, "--mode", mode, "--json", "--allow-fail", output],
    {
      cwd: projectRoot,
      encoding: "utf8",
  ...
  if (child.status !== 0)
    throw new Error(
      `check failed for ${basename(output)}\n${child.stderr || child.stdout}`,
    );
...
async function main() {
  ...
  if (base.segments.length > 1) {
    console.error(
      "[warn] minimizing first segment only; later segments are emptied",
    );
  }
  ...
  const failsAt = (length) => {
    ...
    console.error(
      `[${length}/${max}] ${result.failed ? "FAIL" : "PASS"} ${result.result?.classification || ""}`,
    );
  ...
  if (!failsAt(max).failed) {
    console.log(
      JSON.stringify(
        { minimized: false, reason: "full trace passes", max },
        null,
        2,
      ),
    );
