...
for (const file of files) {
  ...
  for (const spec of forbiddenRuntimeImports) {
    ...
    const importRe = new RegExp(`\\bfrom\\s*['"]${escaped}['"]|\\bimport\\s*\\(\\s*['"]${escaped}['"]\\s*\\)`);
  ...
  for (const match of text.matchAll(/\bfrom\s*['"]([^'"]+)['"]|\bimport\s*\(\s*['"]([^'"]+)['"]\s*\)/g)) {
    ...
    const target = normalize(relative(projectRoot, resolve(dirname(file), spec)));
    const allowed =
      target.startsWith(`js/`) ||
      target.startsWith(`frozen/`) ||
      target.startsWith(`nethack-c/upstream/`);
    if (!allowed) {
      errors.push(`${rel}: relative import escapes submitted runtime surface (${spec})`);
    }
  }
  ...
  if (/\bnew\s+Function\b|\bFunction\s*\(/.test(text)) {
    errors.push(`${rel}: Function constructor is not allowed in submitted runtime code`);
  }
  if (/\bprocess\s*\./.test(text)) {
    errors.push(`${rel}: process.* host access is not allowed in submitted runtime code`);
  }
}
...
console.log(`Submission constraint check passed (${files.length} files scanned).`);
