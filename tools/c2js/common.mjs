...
export function forbiddenRuntimeHooks(text) {
  return [
    [/(\bfrom\s*['"]node:fs['"]|\bimport\s*\(\s*['"]node:fs['"]\s*\))/, 'node:fs import'],
    [/(\bfrom\s*['"]child_process['"]|\bimport\s*\(\s*['"]child_process['"]\s*\))/, 'child_process import'],
    [/(\bfrom\s*['"]worker_threads['"]|\bimport\s*\(\s*['"]worker_threads['"]\s*\))/, 'worker_threads import'],
    [/\brequire\s*\(/, 'require()'],
    [/\beval\s*\(/, 'eval()'],
