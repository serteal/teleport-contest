...
function parseNethackrcName(rc) {
  ...
  for (const rawLine of rc.split("\n")) {
    ...
    if (!(upper.startsWith("OPTIONS=") || upper.startsWith("OPTIONS =")))
      continue;
...
async function clearStaleState(installDir) {
  ...
  try {
    ...
    await Promise.all(
      entries.map((e) => fs.unlink(path.join(saveDir, e)).catch(() => {})),
    );
  } catch {}
...
async function recordSegment({
  ...
  await done;
  return {
    steps: steps.slice(0, expectedSteps),
...
async function main() {
  ...
  if (argv.length < 1 || argv[0] === "-h" || argv[0] === "--help") {
    console.error(
      "Usage: node scripts/record-session.mjs <input.session.json> [output.session.json]",
    );
    process.exit(2);
  }
  ...
  try {
    ...
    for (let i = 0; i < session.segments.length; i++) {
      ...
      console.error(
        `[seg ${i + 1}/${session.segments.length}] seed=${seg.seed} moves=${(seg.moves || "").length}`,
      );
      ...
      const out = {
        ...
        animation_frames: recorded.animation_frames,
      };
      ...
      for (const k of Object.keys(seg)) {
        if (k === "animation_frames") continue;
